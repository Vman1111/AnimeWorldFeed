...
protocol HTTPSession {
    func dataTask(with url: URL, completionHandler: @escaping (Data?, URLResponse?, (any Error)?) -> Void) -> any HTTPSessionTask
}
...
class URLSessionHTTPClientTests: XCTestCase {
    ...
    private class HTTPSessionSpy: HTTPSession, @unchecked Sendable {
        ...
        private struct Stub {
            let task: any HTTPSessionTask
            let error: (any Error)?
        }
        
        func stub(url: URL, task: any HTTPSessionTask = FakeURLSessionDataTask(), error: (any Error)? = nil) {
