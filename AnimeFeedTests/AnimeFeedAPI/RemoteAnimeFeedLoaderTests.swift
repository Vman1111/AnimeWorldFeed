...
final class RemoteAnimeFeedLoaderTests: XCTestCase {
    ...
    // MARK: Helpers
    private func makeSUT(url: URL = URL(string: "http://a-url.com")!, file: StaticString = #file, line: UInt = #line) -> (sut: RemoteAnimeFeedLoader, client: HTTPClientSpy) {
        let client = HTTPClientSpy()
    ...
    func expect(_ sut: RemoteAnimeFeedLoader, toCompleteWith expectedResult: RemoteAnimeFeedLoader.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        sut.load() { receivedResult in
            ...
            case let (.success(receivedItems), .success(expectedItems)):
                ...
                XCTAssertEqual(receivedItems.pagination, expectedItems.pagination, file: file, line: line)
